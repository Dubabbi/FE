    ...
    const fetchTemplateData = async () => {
      ...
      try {
        const response = await axios.get(
          `https://maeummal.com/template4/get?template4Id=${templateId}`,
          {
            headers: {
        ...
        if (response.data.isSuccess && response.data.data) {
          ...
          const updatedStoryCards = templateData.storyCardEntityList.map(
            (card) => ({
              ...card,
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://maeummal.com/template4/upload",
          formData,
          {
            headers: {
        ...
        if (response.data.isSuccess) {
          ...
          newStoryCards[modalCardIndex].imagePreviewUrl =
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.patch(
        `https://maeummal.com/template4/update?template4Id=${templateId}`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("key")}`,
    ...
    if (confirmDelete) {
      try {
        const response = await axios.delete(
          `https://maeummal.com/template4/delete?template4Id=${templateId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("key")}`,
            },
