...
const Template4Std = () => {
  ...
  const toggleSelectImage = (id) => {
    const cardInfo = templateData.storyCardEntityList.find(
      (card) => card.storyCardId === id
    );
    ...
    if (index === -1) {
      const newImages = [
        ...selectedImages,
        { id, answerNumber: cardInfo.answerNumber },
      ];
  ...
  const submitFirstFeedback = async (userOrder) => {
    try {
      const accessToken = localStorage.getItem("key");
      const response = await axios.post(
        "https://maeummal.com/feedback/createFirst",
        {
          templateId: templateData.templateId,
          answerList: userOrder.map(String),
          studentId: userId,
    ...
    <>
      ...
      <L.LessonWrapper>
        ...
        <C.StoryWrap>
          <C.CardContainer>
            {templateData &&
              templateData.storyCardEntityList.map((card, index) => (
                <C.SelectCard
